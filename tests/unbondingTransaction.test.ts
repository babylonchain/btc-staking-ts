...
describe("Unbonding Transaction", () => {
  ...
  const stakerWithdrawAddress = dataGenerator.getAddressAndScriptPubKey(stakerKeyPair.publicKey).nativeSegwit.address;
  const randomOutputIndex = dataGenerator.getRandomIntegerBetween(0, 10);
  // TODO: Loop withdrawEarlyUnbondedTransaction and withdrawTimelockUnbondedTransaction
